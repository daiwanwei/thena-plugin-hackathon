...
contract LimitOrderPlugin is ILimitOrderPlugin, LimitOrderPayments {
    ...
    modifier onlyPlugin(address pool) {
        address plugin = IBasePluginV1Factory(basePluginFactory).pluginByPool(
            pool
        );
    ...
    function getEpoch(
        ...
        return
            epochs[
                keccak256(abi.encode(pool, tickLower, tickUpper, zeroForOne))
            ];
    }
    ...
    function getTickSpacing(
        address pool
    ) private view returns (int24 tickSpacing) {
    ...
    function withdraw(
        ...
        emit Withdraw(msg.sender, epoch, liquidityDelta);
    }

    function afterInitialize(
        address pool,
        int24 tick
    ) external override onlyPlugin(pool) {
