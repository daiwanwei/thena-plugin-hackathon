...
contract AlgebraBasePluginV1 is
    ...
    IAlgebraPlugin
{
    ...
    bytes32 public constant ALGEBRA_BASE_PLUGIN_MANAGER =
        keccak256("ALGEBRA_BASE_PLUGIN_MANAGER");
    ...
    {
        (price, tick, fee, pluginConfig, , ) = IAlgebraPoolState(pool)
            .globalState();
    }
        ...
        returns (int56 tickCumulative, uint88 volatilityCumulative)
    {
        ...
        VolatilityOracle.Timepoint memory result = timepoints
            .getSingleTimepoint(
                _blockTimestamp(),
                secondsAgo,
                tick,
        ...
        (tickCumulative, volatilityCumulative) = (
    ...
    function changeFeeConfiguration(
        AlgebraFeeConfiguration calldata _config
    ) external override {
        require(
    ...
    function setIncentive(address newIncentive) external override {
        ...
        if (toConnect) {
            accessAllowed =
                msg.sender ==
                IBasePluginV1Factory(pluginFactory).farmingAddress();
        } else {
    ...
    /// @inheritdoc IFarmingPlugin
    function isIncentiveConnected(
        address targetIncentive
    ...
    function afterSwap(
        ...
        if (_incentive != address(0) || limitOrderPlugin != address(0)) {
            ...
            if (limitOrderPlugin != address(0)) {
                ILimitOrderPlugin(limitOrderPlugin).afterSwap(
                    pool,
                    zeroToOne,
                    tick
                );
    ...
    function _writeTimepointAndUpdateFee() internal {
        ...
        (uint16 newLastIndex, uint16 newOldestIndex) = timepoints.write(
            _lastIndex,
            currentTimestamp,
            tick
        );
        ...
        uint16 newFee = _getFeeAtLastTimepoint(
            newLastIndex,
            newOldestIndex,
            tick,
            feeConfig_
