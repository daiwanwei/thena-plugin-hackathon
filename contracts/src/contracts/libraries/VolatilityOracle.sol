...
library VolatilityOracle {
    ...
    function write(
        ...
        unchecked {
            ...
            self[indexUpdated] = _createNewTimepoint(
                last,
                blockTimestamp,
                tick,
                avgTick,
    ...
    function getSingleTimepoint(
        ...
        unchecked {
            ...
            if (target == targetTimepoint.blockTimestamp)
                return targetTimepoint; // we're at the left boundary
            if (samePoint) {
                // if target is newer than last timepoint
                (
                    int24 avgTick,
                    uint16 windowStartIndex
                ) = _getAverageTickCasted(
                        self,
                        target,
                        tick,
                        lastIndex,
            ...
            (uint32 timestampAfter, int56 tickCumulativeAfter) = (
            ...
            targetTimepoint.tickCumulative +=
                ((tickCumulativeAfter - targetTimepoint.tickCumulative) /
                    int56(uint56(timepointTimeDelta))) *
    ...
    function getTimepoints(
        ...
        returns (
            ...
            uint88[] memory volatilityCumulatives
        )
    {
        ...
        unchecked {
            for (uint256 i; i < secondsLength; ++i) {
                ...
                (tickCumulatives[i], volatilityCumulatives[i]) = (
    ...
    function getAverageVolatility(
        ...
        unchecked {
            ...
            bool timeAtLastTimepoint = lastTimepoint.blockTimestamp ==
                currentTime;
            uint88 lastCumulativeVolatility = lastTimepoint
            ...
            if (!timeAtLastTimepoint) {
                lastCumulativeVolatility = _getVolatilityCumulativeAt(
                    self,
                    currentTime,
                    0,
                    tick,
            ...
            if (
                ...
                if (timeAtLastTimepoint) {
                    ...
                    uint32 timeDeltaBetweenPoints = self[windowStartIndex + 1]
                        .blockTimestamp - oldestTimestamp;

                    cumulativeVolatilityAtStart +=
                        ((self[windowStartIndex + 1].volatilityCumulative -
                            cumulativeVolatilityAtStart) *
                            (currentTime - WINDOW - oldestTimestamp)) /
                ...
                return ((lastCumulativeVolatility -
                    cumulativeVolatilityAtStart) / WINDOW); // sample is big enough to ignore bias of variance
            } else if (currentTime != oldestTimestamp) {
                // recorded timepoints are not enough, so we will extrapolate
                uint88 _oldestVolatilityCumulative = self[oldestIndex]
                    .volatilityCumulative;
                ...
                return ((lastCumulativeVolatility -
                    _oldestVolatilityCumulative) / unbiasedDenominator);
            }
        }
    }
    ...
    function _getAverageTickCasted(
        ...
        unchecked {
            (avgTick, windowStartIndex) = (
                int24(_avgTick),
                uint16(_windowStartIndex)
            ); // overflow in uint16(_windowStartIndex) is desired
        }
    }
    ...
    function _getAverageTick(
        ...
        unchecked {
            int56 currentTickCumulative = lastTickCumulative +
                int56(tick) *
                int56(uint56(currentTime - lastTimestamp)); // update with new data
            if (
                !_lteConsideringOverflow(
                    oldestTimestamp,
                    currentTime - WINDOW,
                ...
                return (
                    (currentTickCumulative - oldestTickCumulative) /
                        int56(uint56(currentTime - oldestTimestamp)),
                    oldestIndex
                );
            }
            ...
            if (
                ...
                (
                    tickCumulativeAtStart,
                    windowStartIndex
                ) = _getTickCumulativeAt(
                    self,
                    currentTime,
                    WINDOW,
                    tick,
                ...
                //          ||||||||||||||||||||||
                avgTick =
                    (currentTickCumulative - tickCumulativeAtStart) /
                    int56(uint56(WINDOW));
            }
        }
    }
    ...
    function _lteConsideringOverflow(
        uint32 a,
        uint32 b,
        uint32 currentTime
        ...
        unchecked {
            ...
            if (samePoint) {
                ...
                return (volatilityCumulativeBefore +
                    uint88(
                        _volatilityOnRange(
                            int256(uint256(target - timestampBefore)),
                            tick,
                            tick,
                            beforeOrAt.averageTick,
                            avgTick
                        )
                    ));
            }
            ...
            (uint32 timepointTimeDelta, uint32 targetDelta) = (
                ...
                target - timestampBefore
            );
            return
                volatilityCumulativeBefore +
                ((volatilityCumulativeAfter - volatilityCumulativeBefore) /
                    timepointTimeDelta) *
                targetDelta;
        }
    }
    ...
    function _getTickCumulativeAt(
        ...
        unchecked {
            ...
            (uint32 timestampBefore, int56 tickCumulativeBefore) = (
                beforeOrAt.blockTimestamp,
                beforeOrAt.tickCumulative
            );
            if (target == timestampBefore)
                return (tickCumulativeBefore, _indexBeforeOrAt); // we're at the left boundary
            // since target != timestampBefore, `samePoint` means that target is newer than last timepoint
            if (samePoint)
                return (
                    (tickCumulativeBefore +
                        int56(tick) *
                        int56(uint56(target - timestampBefore))),
                    _indexBeforeOrAt
                ); // if target is newer than last timepoint
            ...
            if (target == timestampAfter)
                return (tickCumulativeAfter, uint16(_indexBeforeOrAt + 1)); // we're at the right boundary
            ...
            return (
                tickCumulativeBefore +
                    ((tickCumulativeAfter - tickCumulativeBefore) /
                        int56(uint56(timepointTimeDelta))) *
                    int56(uint56(targetDelta)),
                _indexBeforeOrAt
            );
        ...
        )
    {
        ...
        if (
            target == currentTime ||
            _lteConsideringOverflow(lastTimepointTimestamp, target, currentTime)
        ) {
        ...
        unchecked {
            ...
            if (!_lteConsideringOverflow(oldestTimestamp, target, currentTime))
                revert targetIsTooOld();
            ...
            if (lastIndex == oldestIndex + 1)
                return (self[oldestIndex], lastTimepoint, false, oldestIndex);
        }
        ...
        )
    {
        unchecked {
            ...
            uint256 right = upperIndex < lowerIndex
                ? upperIndex + UINT16_MODULO
                : upperIndex; // newest timepoint considering one index overflow
        ...
        )
    {
        unchecked {
            ...
            do {
                ...
                bool useHeuristic = firstIteration &&
                    withHeuristic &&
                    left == indexBeforeOrAt + 1;
