...
library OracleLibrary {
    ...
    function lastTimepointMetadata(
        address oracleAddress
    ) internal view returns (uint16 index, uint32 timestamp) {
    ...
    function oldestTimepointMetadata(
        address oracleAddress
    ) internal view returns (uint16 index, uint32 timestamp) {
    ...
    function timepointMetadata(
        ...
        (initialized, timestamp, , , , , ) = IVolatilityOracle(oracleAddress)
            .timepoints(index);
    }
}
